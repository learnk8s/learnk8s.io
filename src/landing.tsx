...
export function render(website: Sitemap, currentNode: LinkedNode<ReturnType<typeof Details>>, siteUrl: string): string {
    ...
    >
        ...
        return (
          <JsonLd<Course>
            ...
            item={{
              ...
              hasCourseInstance: course.events.filter(onlyCityOrOnline).map(
                it =>
                  ({
                    '@type': 'CourseInstance',
      ...
      <section className='bg-evian pt4' id='start'>
        ...
        <p className='lh-copy f4 black-70 measure center tc ph3'>
          The advanced course is made 6 core modules that are designed to last 2 full days. You're recommended to select
          4 optional modules for the third day, but you choose more if you wish.
        </p>
  ...
  function onlyCityOrOnline(event: CourseEvent) {
    return (
      (isVenueOnline(event.location) && event.timezone === currentNode.payload.timezone) ||
      event.location.city === currentNode.payload.location.city
    )
  }
}
