...
export function Register(store: Store) {
  store.dispatch(Action.pages.add(Advanced))
  store.dispatch(
...
function renderPage(state: State) {
  ...
  const openGraph = Selector.openGraphs.selectAll(state).find(it => it.pageId === Advanced.id)
...
setTimeout(ldinsp, 0);
      ...
      <Body>
        {[material.architecture, material.networking, material.advancedNetworking].map((it, index) => {
          return (
            <JsonLd<Course>
              key={index}
              item={{
