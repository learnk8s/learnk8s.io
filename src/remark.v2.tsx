...
export function render(path: string): JSX.Element {
  const assetsPath = dirname(path).replace(resolve('.'), '')
    ...
    .use(remark2rehype, {
      ...
      handlers: {
        ...
        image: (h: h, node: Mdast.Image): Node => {
          ...
          return H('img.db.pv3', { ...props, src: resolve(assetsPath, node.url), alt: node.alt })
        },
  ...
  return (hastTree as any) as JSX.Element
}
